<template>
  <div class="dynamic-form">
    <el-form ref="formRef" :model="formData" :rules="formRules" :label-width="labelWidth" :label-position="labelPosition">
      <el-row gutter="24">
        <el-col v-for="(item, index) in formConfig" :key="index" :span="item.span || span">
...
<!--                v-html="item.render(formData, item)">-->
<!--              -->
<!--            </div>-->
...
<!--            <span v-else>{{ formData[item.prop] }}</span>-->
      ...
      <div class="form-actions" v-if="$slots.actions">
        <slot name="actions" :form-data="formData" :form-ref="formRef"></slot>
      </div>
    </el-form>
  </div>
</template>
